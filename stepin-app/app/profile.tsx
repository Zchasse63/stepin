...
import { HealthSettingsCard } from '../components/HealthSettingsCard';
...
export default function ProfileScreen() {
  ...
  const [showHistoricalImportModal, setShowHistoricalImportModal] = useState(false);
  const [streakFreezeStatus, setStreakFreezeStatus] = useState<StreakFreezeStatus | null>(null);
  const [freezableDate, setFreezableDate] = useState<string | null>(null);
  ...
  // Check notification permission status on mount and when screen focuses
  useEffect(() => {
    const checkPermissionStatus = async () => {
      const status = await getNotificationPermissionStatus();
      setNotificationPermissionStatus(status);

      // Show banner if denied and user has any notifications enabled
      if (status === 'denied' && profile?.notification_settings) {
        const hasAnyEnabled =
          profile.notification_settings.dailyReminder ||
          profile.notification_settings.streakReminder ||
          profile.notification_settings.goalCelebration;
        setShowPermissionBanner(hasAnyEnabled);
      } else {
  ...
  useEffect(() => {
    const loadFreezeStatus = async () => {
      if (user) {
        try {
          const status = await getStreakFreezeStatus(user.id);
          setStreakFreezeStatus(status);

          const freezable = await getFreezableDate(user.id);
          setFreezableDate(freezable);
        } catch (error) {
          console.error('[Profile] Error loading streak freeze status:', error);
          // Set to null so the UI doesn't show the section
          setStreakFreezeStatus(null);
          setFreezableDate(null);
        }
      }
    };
  ...
  const handleUseStreakFreeze = async () => {
    if (!user || !freezableDate) return;

    Alert.alert(
      'Use Streak Freeze?',
      `This will protect your streak for ${new Date(freezableDate).toLocaleDateString()}. You have ${streakFreezeStatus?.available || 0} freeze${(streakFreezeStatus?.available || 0) !== 1 ? 's' : ''} available.`,
      [
        {
          text: 'Cancel',
          style: 'cancel',
        },
        {
          ...
          onPress: async () => {
            try {
              const result = await useStreakFreeze(user.id, freezableDate);

              if (result.success) {
                Alert.alert('Success', 'Streak freeze applied! Your streak is protected.');
                // Reload status
                const newStatus = await getStreakFreezeStatus(user.id);
                setStreakFreezeStatus(newStatus);
                const newFreezable = await getFreezableDate(user.id);
                setFreezableDate(newFreezable);
                loadStats();
              } else {
                Alert.alert('Error', result.error || 'Failed to use streak freeze');
              }
            } catch (error: any) {
              Alert.alert('Error', error.message || 'Failed to use streak freeze');
            }
          },
        },
      ]
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      {streakFreezeStatus && (
        <SettingsSection title="Streak Freezes">
          <View style={styles.streakFreezeCard}>
            <View style={styles.streakFreezeHeader}>
              <View style={styles.streakFreezeIconContainer}>
                <Text style={styles.streakFreezeIcon}>❄️</Text>
              </View>
              <View style={styles.streakFreezeInfo}>
                <Text style={styles.streakFreezeTitle}>
                  {streakFreezeStatus.available} / {streakFreezeStatus.maxAllowed} Available
                </Text>
                <Text style={styles.streakFreezeSubtitle}>
                  {streakFreezeStatus.nextEarnAt
                    ? `Next at ${streakFreezeStatus.nextEarnAt}-day streak`
                    : 'Earn freezes every 7-day streak'}
                </Text>
              </View>
            </View>

            <Text style={styles.streakFreezeDescription}>
              Streak freezes protect your streak when you miss a day. Earn one every 7 consecutive days.
            </Text>

            {freezableDate && streakFreezeStatus.available > 0 && (
              <TouchableOpacity
                style={styles.useFreezeButton}
                onPress={handleUseStreakFreeze}
                accessibilityLabel="Use streak freeze"
                accessibilityRole="button"
              >
                <Text style={styles.useFreezeButtonText}>
                  Use Freeze for {new Date(freezableDate).toLocaleDateString()}
                </Text>
              </TouchableOpacity>
            )}
            ...
            {freezableDate && streakFreezeStatus.available === 0 && (
              <View style={styles.noFreezesContainer}>
                <Text style={styles.noFreezesText}>
                  No freezes available. Keep your streak going to earn more!
                </Text>
              </View>
            )}
          </View>
      ...
      <SettingsSection title="Health Tracking">
        <HealthSettingsCard
          onPermissionGranted={() => {
            // Refresh profile to update health data
            loadProfile();
            loadStats();
          }}
          onShowHistoricalImport={() => setShowHistoricalImportModal(true)}
        />
      </SettingsSection>

      {/* Preferences Section */}
      <SettingsSection title="Preferences">
        {/* Permission Banner - show if denied */}
      ...
      {user && (
        <HistoricalImportModal
          visible={showHistoricalImportModal}
          userId={user.id}
          onClose={() => setShowHistoricalImportModal(false)}
          onComplete={(daysImported) => {
